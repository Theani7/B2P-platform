...
**Performance & maintainability refactor (post-FS6):**
- **Bundle:** `/business/campaigns` 294→155 kB (jsPDF/autoTable dynamic-imported on PDF click; `ExportButton` itself `next/dynamic`), `/business/dashboard` 271→152 kB + `/admin/analytics` 257→146 kB (recharts extracted to `components/charts/BusinessDashboardCharts.tsx` + `AdminAnalyticsCharts.tsx`, loaded via `next/dynamic` `ssr:false`), auth pages 214→178 kB (`framer-motion` removed everywhere — replaced with CSS keyframes in `globals.css`: `animate-fade-slide-up/-in/fade-in/pop-in` + delay classes; dependency dropped from package.json).
- **Network:** sidebar `useConversations` now gated to `/messages` (`enabled` option added to the hook) — previously it polled every 15s on every app page **and** auto-created conversation rows; raw `fetch` sidebar prefetch replaced with `queryClient.prefetchQuery` using the real query keys (`["business-profile"]`, `["campaign-dashboard-stats"]`, `["promoter-profile"]`, `["profile-completion"]`, `["admin-dashboard"]`) so the cache is shared with page queries.
- **Maintainability:** merged the duplicate `InvitePromoterModal` (kept `components/discovery/InvitePromoterModal.tsx`, deleted `components/InvitePromoterModal.tsx`); `timeAgo`/`timeStr` consolidated in `lib/time.ts`; new `getApiError(err, fallback)` in `lib/apiError.ts` + `notifyApiError` in `lib/notify.ts` (applied to the common `err?.response?.data?.message` patterns); `messages/page.tsx` split into `components/chat/{MessageBubble,ConversationSidebar,ChatPanel}.tsx` (`browser-image-compression` + apiClient now dynamic-imported inside ChatPanel).
- **Backend:** `matching/service.js` fetches only scoring fields (`select`) and persists match upserts in batches of 100 (was one unbounded `findMany` + one giant transaction); `chat/service.js` `getConversations` now computes exact unread counts via a single `groupBy` query and fetches only the latest message per conversation (was fetching every message body across all conversations).
- **Tooling:** `@next/bundle-analyzer` devDep added; `next.config.mjs` wraps with `withBundleAnalyzer` gated on `ANALYZE=true` → run `ANALYZE=true npm run build` to get the visual bundle report.
- **Known pre-existing issue (not fixed):** frontend `features/auth/api.ts` calls `/auth/n-registration-otp` / `/auth/n-otp` / `/auth/n-reset-code` / `/auth/n-email` but the backend routes are `/auth/verify-registration-otp`, `/verify-otp`, `/verify-reset-code`, `/verify-email` — email-verification flow from the UI is broken.
