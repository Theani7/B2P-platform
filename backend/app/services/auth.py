...
def reset_password(db: Session, payload: ResetPasswordSchema) -> None:
    user = db.query(User).filter(User.reset_code == payload.token).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invalid code")
    if user.reset_code_expiry and datetime.now(timezone.utc) > user.reset_code_expiry.replace(tzinfo=timezone.utc):
