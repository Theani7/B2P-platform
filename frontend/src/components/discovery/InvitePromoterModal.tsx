...
import { useInvitePromoter, useBusinessInvitations, useBusinessApplications } from "../../features/collaboration/api";
import { notifySuccess, notifyError } from "../../hooks/useToast";
import { ApplicationStatus, InvitationStatus } from "../../features/collaboration/types";
...
export default function InvitePromoterModal({ isOpen, onClose, promoter }: InvitePromoterModalProps) {
  ...
  const alreadyInvitedCampaignIds = useMemo(() => {
    return new Set(
      invitations
        .filter((inv: any) => inv.promoter_profile_id === promoter.id && inv.status === InvitationStatus.PENDING)
        .map((inv: any) => inv.campaign_id)
    );
  }, [invitations, promoter.id]);

  const alreadyAppliedCampaignIds = useMemo(() => {
    return new Set(
      applications
        .filter((app: any) => app.promoter_profile_id === promoter.id && app.status === ApplicationStatus.PENDING)
        .map((app: any) => app.campaign_id)
    );
  }, [applications, promoter.id]);
  ...
  const handleSend = () => {
    ...
    const hasExistingInvite = alreadyInvitedCampaignIds.has(selectedCampaignId);
    const hasExistingApp = alreadyAppliedCampaignIds.has(selectedCampaignId);

    if (hasExistingInvite) {
      notifyError(`You have already invited ${promoter.username} to this campaign. Please wait for their response.`);
      return;
    }
    if (hasExistingApp) {
      notifyError(`${promoter.username} has already applied to this campaign. You can accept their application instead.`);
      return;
    }
