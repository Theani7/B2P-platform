...
export default function LandingHowItWorks() {
  return (
    <section id="how-it-works" className="py-20 lg:py-28 bg-stone-100">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20">
          <div>
            <p className="text-xs font-medium uppercase tracking-widest text-brand-purple mb-6">Businesses</p>
              ...
              {businessSteps.map((step, idx) => (
                <motion.div key={step.title} variants={item} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <span className="w-8 h-8 rounded-full bg-brand-purple-50 text-brand-purple-900 border border-purple-200 flex items-center justify-center text-xs font-medium">
                      {idx + 1}
                    </span>
                    {idx < businessSteps.length - 1 && (
                      <span className="w-px flex-1 bg-stone-100 my-2 min-h-[24px]" />
                    )}
                  </div>
                  <div className="pb-6">
                    <p className="text-sm font-medium text-stone-900 mb-1">{step.title}</p>
                    <p className="text-sm text-stone-900 leading-relaxed">{step.desc}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
          ...
          <div>
            ...
            <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="flex flex-col gap-4">
              {promoterSteps.map((step, idx) => (
                <motion.div key={step.title} variants={item} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <span className="w-8 h-8 rounded-full bg-brand-teal-50 text-brand-teal-900 border border-teal-200 flex items-center justify-center text-xs font-medium">
                      {idx + 1}
                    </span>
