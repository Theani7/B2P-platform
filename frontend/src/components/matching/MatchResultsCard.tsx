import type { MatchResultRead } from "../../features/matching/types";

const CLASSIFICATION_STYLES: Record<string, string> = {
  EXCELLENT_MATCH: "bg-brand-teal-50 text-brand-teal-900",
  GOOD_MATCH: "bg-brand-purple-50 text-brand-purple-900",
  AVERAGE_MATCH: "bg-brand-amber-50 text-brand-amber-900",
  LOW_MATCH: "bg-gray-100 text-gray-500",
};

interface MatchResultsCardProps {
  match: MatchResultRead;
  onInvite?: (promoterId: string) => void;
}

export default function MatchResultsCard({ match, onInvite }: MatchResultsCardProps) {
  const scoreColor =
    match.score >= 85
      ? "text-brand-teal"
      : match.score >= 70
        ? "text-brand-amber"
        : "text-gray-500";

  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 space-y-3">
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-3">
          <div className="h-12 w-12 rounded-full bg-brand-purple-50 flex items-center justify-center text-brand-purple-900 font-medium text-lg">
            {match.promoter.avatar_url ? (
              <img src={match.promoter.avatar_url} alt="" className="h-12 w-12 rounded-full object-cover" />
            ) : (
              match.promoter.username.charAt(0).toUpperCase()
            )}
          </div>
          <div>
            <h3 className="text-sm font-medium text-gray-900">{match.promoter.username}</h3>
            {match.promoter.headline && (
              <p className="text-xs text-gray-500">{match.promoter.headline}</p>
            )}
            <p className="text-xs text-gray-400">{match.promoter.niche}</p>
          </div>
        </div>
        <div className="flex items-center space-x-3">
          <div className="text-right">
            <div className={`text-2xl font-medium ${scoreColor}`}>{Math.round(match.score)}</div>
            <span className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${CLASSIFICATION_STYLES[match.classification] || ""}`}>
              {match.classification.replace(/_/g, " ")}
            </span>
          </div>
        </div>
      </div>

      <div className="flex flex-wrap gap-4 text-xs text-gray-500">
        <span>{match.promoter.followers_count.toLocaleString()} followers</span>
        <span>{match.promoter.engagement_rate}% engagement</span>
        {match.promoter.years_experience != null && <span>{match.promoter.years_experience} yrs exp</span>}
        {match.promoter.location && <span>{match.promoter.location}</span>}
        {match.promoter.verified && <span className="text-brand-teal font-medium">Verified</span>}
      </div>

      <p className="text-sm text-gray-700 leading-relaxed">{match.explanation}</p>

      {onInvite && (
        <button
          onClick={() => onInvite(match.promoter.id)}
          className="bg-brand-indigo text-white rounded-lg px-4 py-2 text-sm font-medium hover:opacity-90 transition-opacity"
        >
          Invite
        </button>
      )}
    </div>
  );
}
