...
describe("ScorePill", () => {
  ...
  test("applies teal styling for score >= 85", () => {
    render(<ScorePill score={90} />);
    const pill = screen.getByText("90");
    expect(pill.className).toContain("bg-brand-teal-50");
    expect(pill.className).toContain("text-brand-teal-900");
  });

  test("applies amber styling for score 70-84", () => {
    render(<ScorePill score={75} />);
    const pill = screen.getByText("75");
    expect(pill.className).toContain("bg-brand-amber-50");
    expect(pill.className).toContain("text-brand-amber-900");
  });
  ...
  test("applies teal styling at exact threshold of 85", () => {
    render(<ScorePill score={85} />);
