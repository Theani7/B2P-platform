      ...
      <div className="max-w-[1200px] mx-auto space-y-8 pb-20">
        ...
        {tab === "received" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="rounded-cards-lg border border-steel/10 bg-gradient-to-b from-amber-tag/10 to-white p-8 shadow-product-card flex flex-col items-center justify-center text-center">
            ...
            <div className="rounded-cards-lg border border-steel/10 bg-white p-8 shadow-product-card flex flex-col items-center justify-center text-center">
          ...
          {active.isLoading ? (
            ...
            <div className="bg-white rounded-cards border border-slate-custom/10 shadow-product-card p-16 text-center flex flex-col items-center">
              ...
              <p className="text-sm text-ash max-w-md mb-8">
                {tab === "received"
                  ? "You haven't received any reviews from brands yet. Complete collaborations to start building your reputation."
                  : "You haven't reviewed any brands yet. Complete a collaboration to leave a review."}
              </p>
              <Link href="/promoter/collaborations" className="bg-signal-blue text-white rounded-inputs px-6 h-12 flex items-center justify-center text-sm font-bold hover:bg-signal-blue/90 transition-colors shadow-sm">
                View Collaborations
              </Link>
            </div>
          ) : (
            <div className="space-y-4">
              {items.map((r) => {
                const partnerName = r.businessName || r.reviewer?.username || "Brand";
                return (
                  <div key={r.id} className="rounded-cards-lg border border-slate-custom/10 bg-white p-6 shadow-product-card transition-all hover:-translate-y-0.5 hover:border-signal-blue/30 hover:shadow-feature-section">
                    <div className="flex items-start justify-between gap-4 mb-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-base font-bold text-graphite">{partnerName}</h3>
                    ...
                    {tab === "given" && (
                      <div className="flex items-center gap-3 pt-5 border-t border-slate-custom/10 mt-5">
                        ...
                        <Link href="/promoter/collaborations" className="ml-auto text-signal-blue font-bold text-sm hover:underline flex items-center gap-1">
