...
function Dialog({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-graphite/50 backdrop-blur-sm">
      <div className="bg-white rounded-cards-lg w-full max-w-md overflow-hidden relative" role="dialog" aria-modal="true">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-custom/10">
          <h2 className="text-heading text-graphite">{title}</h2>
          <button onClick={onClose} className="p-1.5 text-ash hover:text-graphite hover:bg-sky-wash rounded-button transition-colors">
            <XCircle size={18} />
          </button>
        </div>
...
function VerificationRequests() {
  ...
  const confirmApprove = () => {
    ...
    approve.mutate(approveConfirm, {
      onSuccess: () => {
        notifySuccess("Verification approved");
  ...
  const confirmReject = () => {
    ...
    reject.mutate(
      { id: rejectDialog.requestId, adminNotes: rejectDialog.notes || undefined },
      {
        onSuccess: () => {
          notifySuccess("Verification rejected");
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case "PENDING":
        return { bg: "bg-amber-tag/10", text: "text-amber-tag", dot: "bg-amber-tag" };
      case "APPROVED":
        return { bg: "bg-emerald-status/10", text: "text-emerald-status", dot: "bg-emerald-status" };
      case "REJECTED":
        return { bg: "bg-coral-alert/10", text: "text-coral-alert", dot: "bg-coral-alert" };
      default:
        return { bg: "bg-slate-custom/10", text: "text-graphite", dot: "bg-graphite" };
    }
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto space-y-8 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {data.items.map((req) => {
            ...
            return (
              <div
                key={req.id}
                className="bg-white border border-slate-custom/10 border-t border-t-signal-blue rounded-cards shadow-product-card p-5 flex flex-col h-full hover:border-signal-blue/30 transition-all"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-sm bg-slate-custom/10 text-ash uppercase tracking-widest">
                          {req.requester_type}
                        </span>
                      </div>
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-badges ${statusStyle.bg} px-2 py-1 text-caption font-medium ${statusStyle.text} uppercase tracking-wide`}
                  >
      ...
      {data && data.pages > 1 && (
        <div className="p-4 bg-white border border-slate-custom/10 rounded-cards flex items-center justify-between shadow-product-card">
          <button
            ...
            disabled={page <= 1}
            className="rounded-buttons border border-slate-custom/10 bg-white px-4 py-2 text-body font-medium text-graphite hover:bg-sky-wash disabled:opacity-50 transition-colors"
          >
            Previous
          </button>
      ...
      <Dialog isOpen={!!approveConfirm} onClose={() => setApproveConfirm(null)} title="Approve Verification">
        <p className="text-body text-graphite">
          Are you sure you want to approve this verification request? This will mark the promoter as verified across the platform.
        </p>
        <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-custom/10">
          <button
            onClick={() => setApproveConfirm(null)}
            className="rounded-buttons border border-slate-custom/10 bg-white px-5 py-2.5 text-body font-medium text-graphite hover:bg-sky-wash transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={confirmApprove}
            className="rounded-buttons bg-emerald-status border border-emerald-status px-5 py-2.5 text-body font-medium text-white hover:bg-emerald-status/90 transition-colors"
          >
            Confirm Approval
          </button>
        </div>
      ...
      <Dialog isOpen={!!rejectDialog} onClose={() => setRejectDialog(null)} title="Reject Verification">
