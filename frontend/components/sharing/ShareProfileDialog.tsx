...
import { useProfileShare } from "@/features/sharing/api";
import { useAuth } from "@/providers/AuthProvider";
import { Role } from "@/lib/roles";

export function ShareProfileDialog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const { data: shareInfo, isLoading } = useProfileShare();
  const { user } = useAuth();
  ...
  const handleCopy = async () => {
    if (shareInfo?.publicUrl) {
      await navigator.clipboard.writeText(shareInfo.publicUrl);
      setCopied(true);
  ...
  const handleNativeShare = async () => {
    if (shareInfo?.publicUrl && navigator.share) {
      try {
        await navigator.share({
          title: `Check out ${shareInfo.username} on Byparsathy`,
          url: shareInfo.publicUrl,
        });
      } catch {
        ...
        <div className="p-6">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="mb-5">
                <label className="block text-xs font-medium text-ash uppercase tracking-wider mb-2">Public Link</label>
                <div className="flex items-center gap-2 p-2.5 bg-linen-canvas border border-slate-custom/10 rounded-inputs">
                  <span className="flex-1 text-sm text-graphite font-mono truncate">{shareInfo.publicUrl}</span>
                  <button
                    onClick={handleCopy}
