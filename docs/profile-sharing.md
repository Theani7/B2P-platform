...
## 1. Overview
The Public Profile Sharing module (PH-2.12) gives Promoters and Businesses the ability to quickly distribute their public `Byparsathy` footprint via standard links and custom QR Codes.
...
- When a user downloads the QR code, the browser converts the `<svg>` object into a `<canvas>`, injects `Byparsathy` branding, and downloads it locally as a PNG.
